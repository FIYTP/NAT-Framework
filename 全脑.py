...
MODEL_CONFIG = {
    "name": "Llama2 7B (4bit)",
    "path": r"E:\AI_Models\Llama2 7B\models",
    "layers": 32,
...
class ActivationCollector:
    ...
    def _load_model(self):
        ...
        # 加载模型
        if self.device != "cuda":
            print("⚠️ 4bit quantization requires GPU, using CPU mode (very slow)")
            ...
            def hook(module, input, output):
                if output is not None:
                    ...
                    if isinstance(output, tuple):
                        act = output[0][0, -1, :].detach().cpu().float()
                    else:
        ...
        for layer_idx in range(self.n_layers):
            ...
            if hasattr(target_layer, 'mlp'):
                ...
                if hasattr(mlp, 'act_fn'):
                    hook = mlp.act_fn.register_forward_hook(make_hook(layer_idx))
                    self._active_hooks.append(hook)
                elif hasattr(mlp, 'gate_proj'):
                    hook = mlp.gate_proj.register_forward_hook(make_hook(layer_idx))
                    self._active_hooks.append(hook)

    def collect_average_activations(self, text: str, num_iterations: int = 5) -> Dict:
        """收集单个文本的平均激活值（Llama2较大，减少默认迭代次数）"""
    ...
    def collect_multiple_texts(self, texts: List[str], num_iterations: int = 5) -> Dict:
        """收集多个文本的平均激活值（按层平均）"""
        # 初始化累积器
        all_layers = set()
    ...
    def save_to_csv(self, activations: Dict, description: str,
                    custom_filename: str = "", output_dir: str = "results_llama2") -> str:
        """保存激活值到CSV"""
        ...
        if custom_filename:
            filename = f"llama2_{custom_filename}.csv"
        else:
            filename = f"llama2_activations_{description}_{timestamp}.csv"
...
class EnhancedActivationComparator:
    ...
    def compare_with_baseline(baseline_file: str, target_file: str,
                              output_dir: str = "comparison_results_llama2") -> Tuple[pd.DataFrame, plt.Figure, str, str]:
        """
        ...
        for neuron in top_neurons:
            ...
            for _, row in neuron_data.iterrows():
                detailed_data.append({
                    ...
                    "percent_change": float((row["difference"] / (abs(row["average_activation_baseline"]) + 1e-8)) * 100)
                })
        ...
        # 保存详细CSV
        csv_filename = f"llama2_comparison_{target_name}_vs_{base_name}_{timestamp}.csv"
        ...
        # 保存热图
        heatmap_filename = f"llama2_heatmap_{target_name}_vs_{base_name}_{timestamp}.png"
    ...
    def _create_heatmap(matrix, top_neurons, all_layers, base_name, target_name, merged):
        ...
        # 创建图表 - Llama2神经元多，调整大小
        fig_width = max(14, num_layers * 0.8)
        fig_height = max(10, num_neurons * 0.25)
        ...
        ax.set_xticklabels([f"L{layer}" for layer in all_layers], fontsize=6, rotation=45)
        ax.set_yticklabels([f"N{neuron}" for neuron in top_neurons], fontsize=5)

        ax.set_xlabel("Layer", fontsize=9)
        ax.set_ylabel("Neuron ID", fontsize=9)

        ax.set_title(f"Llama2 7B: Activation Differences (Seed=42)\n{target_name} vs {base_name}",
...
class GradioApp:
    ...
    def create_interface(self):
        with gr.Blocks(title=f"{MODEL_CONFIG['name']} - Random Text Test", theme=gr.themes.Soft()) as app:
            ...
            gr.Markdown(f"### {MODEL_CONFIG['layers']} Layers | {MODEL_CONFIG['neurons_per_layer']:,} Neurons/Layer | 4bit Quantized")
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    num_iterations = gr.Slider(
                        ...
                        value=3,
                        step=1,
                        info="Llama2 is large, use fewer iterations"
                    )
...
def main():
    ...
    print(f"  • Total neurons: {MODEL_CONFIG['layers'] * MODEL_CONFIG['neurons_per_layer']:,}")
