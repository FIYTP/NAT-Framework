...
class ActivationCollector:
    ...
    def collect_average_activations(self, text: str, num_iterations: int = 10) -> Dict:
        ...
        for layer_key in self.activations_sum:
            ...
            if count > 0:
                ...
                avg_activations[layer_key] = np.zeros_like(sum_act.numpy())
    ...
    def save_to_csv(self, activations: Dict, description: str,
                    custom_filename: str = "", output_dir: str = "results") -> str:
        """保存激活值到CSV"""
        ...
        if custom_filename:
            ...
            filename = f"activations_{description}_{timestamp}.csv"
...
class EnhancedActivationComparator:
    ...
    def compare_with_baseline(baseline_file: str, target_file: str,
                              output_dir: str = "comparison_results") -> Tuple[pd.DataFrame, plt.Figure, str, str]:
        """
        ...
        # 保存详细CSV
        csv_filename = f"comparison_{target_name}_vs_{base_name}_{timestamp}.csv"
        ...
        # 保存热图
        heatmap_filename = f"heatmap_{target_name}_vs_{base_name}_{timestamp}.png"
    ...
    def _create_heatmap(matrix, top_neurons, all_layers, base_name, target_name, merged):
        ...
        ax.set_title(f"Activation Differences (Seed=42)\n{target_name} vs {base_name}",
...
class GradioApp:
    ...
    def create_interface(self):
        with gr.Blocks(title="Neuron Activation Analyzer - Random Text Test", theme=gr.themes.Soft()) as app:
            gr.Markdown("# 🧠 Neuron Activation Analyzer - Random Text Test")
            gr.Markdown("### 1. Upload JSON → 2. Select baseline → 3. Run random test → Auto compare")
...
# ==================== 主程序 ====================
def main():
    MODEL_PATH = r"E:\AI_Models\Pythia\Pythia-160M\models"
    ...
    print("=" * 70)
    print("🚀 Starting Neuron Activation Analyzer - Random Text Test")
    print("=" * 70)
    print("Features:")
