"""
Neuron Intervention Experiment - Correct Four Module Structure (Llama2 7B 4bit)
...
# ==================== Model Manager - Llama2 7B 4bit ====================
class ModelManager:
    """Manage local Llama2 7B 4bit model with working intervention"""
    ...
    def __init__(self, model_path: str = r"E:\AI_Models\Llama2 7B\models"):
        ...
        self.model = None
        self.tokenizer = None
        self.n_layers = 32  # Llama2 7B有32层
        self.d_mlp = 11008  # Llama2 7B的FFN维度是11008
        self.hidden_size = 4096  # Llama2 7B的隐藏层维度
        self._load_model()

    def _load_model(self):
        """Load local Llama2 7B 4bit model"""
        print("=" * 60)
        print("🦙 Loading Llama2 7B 4bit from LOCAL path")
        print("=" * 60)
        ...
        try:
            ...
            # 检查是否有GPU
            if self.device != "cuda":
                print("⚠️ 4bit quantization requires GPU, using CPU mode (may be slow)")
    ...
    def get_fingerprint(self,
        ...
        """
        Generate fingerprint with WORKING intervention for Llama2
        """
        ...
        if inhibit_neurons and len(inhibit_neurons) > 0:
            ...
            for layer, neuron in inhibit_neurons:
                if 0 <= layer < len(self.model.model.layers):
                    try:
                        target_layer = self.model.model.layers[layer]

                        # 尝试不同的可能位置
                        if hasattr(target_layer, 'mlp') and hasattr(target_layer.mlp, 'act_fn'):
                            hook = target_layer.mlp.act_fn.register_forward_hook(
                                create_inhibition_hook(layer, neuron)
                            )
                            hooks.append(hook)
                            print(f"   ✓ L{layer} N{neuron} (act_fn)")
                        elif hasattr(target_layer, 'mlp') and hasattr(target_layer.mlp, 'gate_proj'):
                            hook = target_layer.mlp.gate_proj.register_forward_hook(
                                create_inhibition_hook(layer, neuron)
                            )
                            hooks.append(hook)
                            print(f"   ✓ L{layer} N{neuron} (gate_proj)")
                        else:
                            print(f"   ⚠️ Could not find activation layer for L{layer}")
                    except Exception as e:
        ...
        def create_capture_hook(layer_idx):
            def hook(module, input, output):
                if output is not None:
                    # 取最后一个token的激活值
                    if isinstance(output, tuple):
                        last_token_activation = output[0][0, -1, :].detach().cpu().float().numpy()
                    else:
                        last_token_activation = output[0, -1, :].detach().cpu().float().numpy()
        ...
        for layer in monitor_layers:
            if 0 <= layer < len(self.model.model.layers):
                target_layer = self.model.model.layers[layer]
                if hasattr(target_layer, 'mlp') and hasattr(target_layer.mlp, 'act_fn'):
                    hook = target_layer.mlp.act_fn.register_forward_hook(create_capture_hook(layer))
                    capture_hooks.append(hook)
                elif hasattr(target_layer, 'mlp') and hasattr(target_layer.mlp, 'gate_proj'):
                    hook = target_layer.mlp.gate_proj.register_forward_hook(create_capture_hook(layer))
                    capture_hooks.append(hook)
...
# ==================== MODULE 1: MONITOR SETTINGS (Llama2) ====================
class MonitorModule:
        ...
        with gr.Column():
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Markdown(
                        f"**Model:** Llama2 7B (4bit) | **Layers:** 0-{self.n_layers - 1} | **Neurons/layer:** {self.d_mlp} | **Total:** {self.n_layers * self.d_mlp:,}"
                    )
...
# ==================== MODULE 2: INTERVENTION SETTINGS (Llama2) ====================
...
# ==================== MODULE 3: FINGERPRINT GENERATION (Llama2) ====================
class FingerprintModule:
    ...
    def generate_and_save(self,
        ...
        try:
            ...
            filename = f"llama2_{fp_type.lower()}_{clean_name}_{timestamp}.csv"

            save_dir = Path("llama2_fingerprints")
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        repeats_slider = gr.Slider(
                            ...
                            value=2,
                            step=1,
                            info="Llama2 is larger, use fewer repeats"
                        )
...
class ComparisonModule:
    ...
    def calculate_similarity(self, fp1_path: str, fp2_path: str) -> Dict:
        ...
        try:
            ...
            meta1_path = fp1_path.replace("llama2_", "metadata_").replace(".csv", ".json")
            meta2_path = fp2_path.replace("llama2_", "metadata_").replace(".csv", ".json")
...
# ==================== Main Application (Llama2) ====================
class NeuronInterventionApp:
    """Main application - Four module structure with working intervention for Llama2"""

    def __init__(self, model_path: str):
        print("\n" + "=" * 60)
        print("🦙 LLAMA2 7B NEURON INTERVENTION EXPERIMENT SYSTEM")
        print("=" * 60)
        ...
        print(
            f"📊 Model: 32 layers, {self.model_manager.d_mlp if hasattr(self, 'model_manager') else 11008} neurons/layer")
        print("=" * 60)
    ...
    def get_fingerprint_files(self):
        """Get list of saved fingerprint files"""
        fingerprint_dir = Path("llama2_fingerprints")
        if not fingerprint_dir.exists():
            fingerprint_dir.mkdir(exist_ok=True)
        return list(fingerprint_dir.glob("llama2_*.csv"))

    def create_interface(self):
        """Create main interface"""
        with gr.Blocks(title="🦙 Llama2 7B Neuron Intervention System", theme=gr.themes.Soft()) as app:
            gr.Markdown("# 🦙 Llama2 7B Neuron Intervention Experiment System")
            gr.Markdown("### Four-Module Structure: Monitor → Intervene → Generate → Compare")
            gr.Markdown("### ✅ 4bit Quantized | 32 Layers | 11,008 Neurons/Layer")
            ...
            def load_file_info(filepath):
                ...
                meta_path = str(filepath).replace("llama2_", "metadata_").replace(".csv", ".json")
...
# ==================== Main ====================
def main():
    MODEL_PATH = r"E:\AI_Models\Llama2 7B\models"
    ...
    try:
        print("=" * 70)
        print("🦙 Llama2 7B Neuron Intervention System (4bit Quantized)")
        print("=" * 70)
        print(f"📁 Model path: {MODEL_PATH}")
        print(f"📊 Model specs: 32 layers, 11008 neurons/layer")
        print(f"📈 Total neurons: 352,256")
        print("✅ Using file explorer for fingerprint selection")
        print("📁 Fingerprints saved to: ./llama2_fingerprints/")
        print("=" * 70)
        ...
        print("\n" + "=" * 70)
        print("✅ System ready!")
        print("   - 4bit quantized Llama2 7B")
        print("   - Tensor cloning for intervention")
        print("   - File explorer for fingerprint selection")
        print("=" * 70 + "\n")
